...
def goodness_of_fit_par(learner, arrivals, step, method):
    ...
    residuals = [resid(x, intensities, arrivals, dim, method)
...
def plot_resid(resid, rows, cols):
    ...
    ax.set_title('Goodness-of-fit for nonparametric HP')
    k = stats.probplot(resid[0], dist=stats.expon,
                       fit=True, plot=ax, rvalue=False)
    ax.plot(k[0][0], k[0][0], 'k--')
    print(stats.kstest(resid[0][np.logical_not(np.isnan(resid[0]))], 'expon'))
    return fig
...
def goodness_of_fit_nonpar_paralel(nplearner, arrivals, timestep, method):
    ...
    intensities = hp.tracked_intensity
    x = hp.intensity_tracked_times
    residuals = [pool.apply(resid, (x, intensities, arrivals, dim, method))
                 for dim in range(dimension)]
    pool.close()
    return residuals
